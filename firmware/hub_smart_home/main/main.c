...
static bool check_wday(weekday_bits_t weekday)
{
    ...
    if (weekday.monday && temp_time.tm_wday == 1)
        return true;
    if (weekday.tuesday && temp_time.tm_wday == 2)
        return true;
    if (weekday.wednesday && temp_time.tm_wday == 3)
        return true;
    if (weekday.thursday && temp_time.tm_wday == 4)
        return true;
    if (weekday.friday && temp_time.tm_wday == 5)
        return true;
    if (weekday.saturday && temp_time.tm_wday == 6)
        return true;
    if (weekday.sunday && temp_time.tm_wday == 0)
        return true;
    return false;
}
    ...
    while (temp != NULL)
    {
        ...
        if (dev != NULL)
        {
            ...
            if (temp->timeslot.time_on.seconds != temp_time.tm_sec)
                goto check_time_off;
            if (!check_wday(temp->timeslot.weekday))
                goto check_time_off;
            ...
            {
                if (temp->timeslot.channel_1)
                    ctrl_data[1] |= 0x21;
                if (temp->timeslot.channel_2)
                    ctrl_data[1] |= 0x42;
                if (temp->timeslot.channel_3)
                    ctrl_data[1] |= 0x84;
            }
