...
const RelaysDialog = (props) => {
  ...
  const relayNodes = nodes.filter(
    (node) => node.type === "Relay" && node.isADE === true
  );
        ...
        <Stack direction="row" spacing="2px" width="100%">
          <BoxEdit title="Relay" button="Unlink" buttonHandle={deleteHandle}>
            {relay?.state !== undefined && (
              <Switch checked={relay?.state} onChange={handleChangeState} />
            )}
          </BoxEdit>
        </Stack>
      ) : (
